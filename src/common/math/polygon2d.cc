...
double Polygon2d::DistanceTo(const Vec2d& point) const {
    ...
    for (int i = 0; i < num_points_; ++i) { distance = std::min(distance, line_segments_[i].DistanceTo(point)); }
    return distance;
}
...
double Polygon2d::DistanceTo(const LineSegment2d& line_segment) const {
    ...
    for (int i = 0; i < num_points_; ++i) { distance = std::min(distance, line_segment.DistanceTo(points_[i])); }
    return distance;
}
...
double Polygon2d::DistanceTo(const Polygon2d& polygon) const {
    ...
    double distance = std::numeric_limits<double>::infinity();
    for (int i = 0; i < num_points_; ++i) { distance = std::min(distance, polygon.DistanceTo(line_segments_[i])); }
    return distance;
}
...
bool Polygon2d::IsPointOnBoundary(const Vec2d& point) const {
    return std::any_of(line_segments_.begin(), line_segments_.end(), [&](const LineSegment2d& poly_seg) {
...
bool Polygon2d::Contains(const LineSegment2d& line_segment) const {
    ...
    if (!is_convex_) {
        ...
        for (const auto& overlap_seg : overlaps) { total_length += overlap_seg.length(); }
...
bool Polygon2d::Contains(const Polygon2d& polygon) const {
    ...
    return std::all_of(line_segments.begin(), line_segments.end(), [&](const LineSegment2d& line_segment) {
...
void Polygon2d::BuildFromPoints() {
    ...
    area_ = 0.0;
    for (int i = 1; i < num_points_; ++i) { area_ += CrossProd(points_[0], points_[i - 1], points_[i]); }
    if (area_ < 0) {
        area_ = -area_;
    ...
    for (int i = 0; i < num_points_; ++i) { line_segments_.emplace_back(points_[i], points_[Next(i)]); }
...
bool Polygon2d::ComputeConvexHull(const std::vector<Vec2d>& points, Polygon2d* const polygon) {
    ...
    for (int i = 0; i < count; ++i) { result_points.push_back(points[results[i]]); }
...
std::vector<LineSegment2d> Polygon2d::GetAllOverlaps(const LineSegment2d& line_segment) const {
    ...
    for (const auto& poly_seg : line_segments_) {
        Vec2d pt;
        if (poly_seg.GetIntersect(line_segment, &pt)) { projections.push_back(line_segment.ProjectOntoUnit(pt)); }
    }
...
std::string Polygon2d::DebugString() const {
    return ::common::StrCat("polygon2d (  num_points = ",
                            num_points_,
                            "  points = (",
